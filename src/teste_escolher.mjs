...
ok('Própons Lume/Aurora/Ápice com leve/médio/pesado', linhas.length === 3 && /Própons Lume.*Leve · Rápido/.test(linhas[0]) && /Própons Aurora.*Médio/.test(linhas[1]) && /Própons Ápice.*Pesado/.test(linhas[2]), linhas.join(' | '));
