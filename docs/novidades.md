...
- **Memória maior e medida de verdade:** no PC a IA passa a lembrar 16 mil tokens (32 mil com 8 GB de RAM ou mais), no celular 8 mil (16 mil com 8 GB ou mais), gastando só uns 100 MB a mais. Com a conversa cheia, a compactação aparece como um passo ("Compactando a conversa"). O tamanho de cada mensagem é contado pelo próprio modelo, sem estimativa.
- **Esforço Auto, o novo padrão no computador:** a IA raciocina só quando a pergunta pede (contas, código, "por quê") e responde direto no resto. No placar novo, de 145 perguntas, o Auto acerta entre o Médio e o Alto em metade do tempo do Alto. No celular, onde pensar leva mais tempo, Lume e Aurora começam no Médio (o Auto fica a um toque).
...
- **Área de código: um chat para programar.** Fica no menu lateral (abaixo da busca), com a mesma lógica do chat, mas o foco é codificar: você pede em português ("crie um jogo da forca em Python", "leia o main.py e corrija o erro") e a IA **lê os arquivos** e propõe criar, alterar ou apagar — cada mudança aparece com **o que muda linha a linha** e só entra quando você toca em Aplicar. No Windows e no Linux dá para **abrir uma pasta de verdade do computador** e trabalhar nos seus próprios arquivos; nas outras plataformas os arquivos ficam na área do app (e você pode salvar no aparelho ou mandar para o chat). Todo bloco de código das respostas tem "Guardar" para ir direto para lá.
- **A área de código ficou completa:** tem o **histórico dela** (várias conversas de programação, separadas das do chat), **moldes** de pedido prontos para colar (e os seus), **compactação de contexto** (o que já passou vira um resumo curto e cabe mais na memória da IA), **gravação de áudio** na mesma caixa e uma mensagem simples no meio da tela, como no chat normal. O desenho ficou parecido com o do Claude Code: o pedido em fonte de máquina de escrever, cada arquivo lido ou alterado numa linha própria e o **diff com as contagens +/−**.
