...
// Uso: node treino/avaliar.mjs http://127.0.0.1:8765 [chave] [--vezes 3] [--nome lume] [--pensar]
...
const VEZES = +opt('--vezes', 3), NOME = opt('--nome', 'modelo'), PENSAR = args.includes('--pensar');
...
async function perguntar(q, exato) {
  const corpo = { messages: [{ role: 'system', content: SISTEMA }, { role: 'user', content: q.pergunta }], max_tokens: PENSAR ? 1500 : 400, stream: false,
  ...
  return { texto, tokens: j.usage ? j.usage.completion_tokens : 0, ms: Date.now() - t0 };
}
...
let totalAcertos = 0, totalPerguntas = 0, armadilhasOk = 0, armadilhas = 0, repetidas = 0, loops = 0, tokens = 0, ms = 0, geracoes = 0;
console.log(`Placar · ${NOME} · ${banco.length} perguntas × ${VEZES}${PENSAR ? ' · pensando' : ''} · ${url}`);
for (const q of banco) {
  const exato = q.tipo === 'exato' || q.materia === 'matemática' || q.materia === 'programação';
  const respostas = [];
  for (let v = 0; v < VEZES; v++) { const r = await perguntar(q, exato); respostas.push(r.texto); tokens += r.tokens; ms += r.ms; geracoes++; }
...
const resumo = [
  `# Placar — ${NOME} — ${new Date().toISOString().slice(0, 16).replace('T', ' ')}${PENSAR ? ' — pensando' : ''}`, '',
  `- **Acerto (perguntas com resposta):** ${pct(totalAcertos, totalPerguntas)} (${totalAcertos}/${totalPerguntas})`,
  ...
  `- **Velocidade média:** ${(tokens / (ms / 1000)).toFixed(1)} tokens/s · ${(ms / geracoes / 1000).toFixed(1)} s por resposta`, '',
...
const saida = path.join(aqui, 'avaliacao', `resultado-${NOME}-${new Date().toISOString().slice(0, 10)}.md`);
