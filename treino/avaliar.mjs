...
// Uso: node treino/avaliar.mjs http://127.0.0.1:8765 [chave] [--vezes 3] [--nome lume] [--pensar | --auto] [--app]
//   --app: exatamente como o app pergunta (1.27+): texto de sistema fixo (sem o "sobre o app"), o que muda por pergunta
//          num bloco <contexto> na pergunta, limites de tokens do app (1500 / 4500 pensando) e as contas conferidas
//          pelo app (src/calc.js) antes de corrigir. É o modo que mede o que o aluno vê.
...
const VEZES = +opt('--vezes', 3), NOME = opt('--nome', 'modelo'), PENSAR = args.includes('--pensar'), AUTO = args.includes('--auto'), APP = args.includes('--app');
const aqui = path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1'));
const banco = JSON.parse(fs.readFileSync(path.join(aqui, 'avaliacao', 'banco.json'), 'utf8'));
const SISTEMA_TUDO = fs.readFileSync(path.join(aqui, '..', 'src', 'conhecimento.md'), 'utf8');
// como o app: a parte "Sobre você (a Própons IA)" só vai quando a pergunta é sobre o app (no placar: as de identidade)
const MARCA_SOBRE = 'Sobre você (a Própons IA)', iSobre = SISTEMA_TUDO.indexOf(MARCA_SOBRE);
const SISTEMA = APP && iSobre > 0 ? SISTEMA_TUDO.slice(0, iSobre).trim() : SISTEMA_TUDO;
const SOBRE_APP = iSobre > 0 ? SISTEMA_TUDO.slice(iSobre).trim() : '';
const INSTRUCAO_ALTO = 'Antes de responder, pense rápido e objetivo: veja o que foi pedido, resolva e confira. Poucas linhas de raciocínio, sem repetir a pergunta, e então responda.';
// o mesmo código do app: decisão do esforço Auto e busca por trechos no documento longo
const app = vm.createContext({});
vm.runInContext(['detecta.js', 'busca.js', 'calc.js'].map(f => fs.readFileSync(path.join(aqui, '..', 'src', f), 'utf8')).join('\n') + ';this.precisaPensar = precisaPensar; this.CALC = CALC;', app);
...
async function perguntar(q, exato) {
  ...
  let pergunta = conteudo(q);
  if (APP) {   // o bloco <contexto> do app: instrução do esforço (Auto pensando = Alto) e o "sobre o app" quando é sobre ele
    const ctx = [pensar ? INSTRUCAO_ALTO : '', q.materia === 'identidade' ? SOBRE_APP : ''].filter(Boolean).join('\n\n');
    if (ctx) pergunta = '<contexto>\n' + ctx + '\n</contexto>\n\n' + pergunta;
  }
  const corpo = { messages: [{ role: 'system', content: SISTEMA }, { role: 'user', content: pergunta }], max_tokens: APP ? (pensar ? 4500 : 1500) : pensar ? 1500 : 400, stream: false,
  ...
  let texto = String(j.choices[0].message.content || '').replace(/<think>[\s\S]*?(<\/think>|$)/g, '').trim();
  if (APP) texto = app.CALC.conferirContas(texto).texto;   // o app refaz as contas simples antes de mostrar
...
const MODO = (PENSAR ? ' · pensando' : AUTO ? ' · auto' : '') + (APP ? ' · app' : '');
...
const resumo = [
  `# Placar — ${NOME} — ${new Date().toISOString().slice(0, 16).replace('T', ' ')}${MODO.replace(/ · /g, ' — ')}`, '',
...
const saida = path.join(aqui, 'avaliacao', `resultado-${NOME}${MODO.replace(/ · /g, '-')}-${new Date().toISOString().slice(0, 10)}.md`);
